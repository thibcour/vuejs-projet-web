<template>
  <div class="modal-content">
    <div class="modal-header">
      <h5 class="modal-title">Ajouter un produit Collection du Site</h5>
...
export default {
  ...
  methods: {
    closeAndRedirect() {
      ...
      this.router.push({ name: 'ProductManagement' }); // Change 'product-management' to 'ProductManagement'
    },
    UpdateValues(e) {
    ...
    addProduct: async function() {
      ...
      const productRef = dbRef(db, `products/${category}/${this.newProduct.name}`);
      set(productRef, {
